...
class ConvertPanelWaiting extends React.Component {
  static propTypes = {
  ...
  render() {
    const { estimate, eth, hash, rates } = this.props
      ...
      <Container>
        ...
        <Subtitle>
          {hash
            ...
            : `Confim this conversion in ${web3Provider}`}
        </Subtitle>

        <Message>
          Converting <EthValue>{toWei(eth)}</EthValue> into approximately{' '}
...
const mapStateToProps = state => ({
  ...state.convertForm,
  rates: state.rates,
  hash: state.convertPanel.ongoingTx.hash
})

export default connect(mapStateToProps)(ConvertPanelWaiting)
