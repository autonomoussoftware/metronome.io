import converterLogsParser from 'metronome-converter-logs-parser'
...
class ConvertPanelReceipt extends React.Component {
  static propTypes = {
  ...
  render() {
    ...
    const converterLog = converterLogsParser(logs).find(
      log => log.decoded.from === from
    )

    const value = new BigNumber(converterLog.decoded.eth).toString()
    ...
    return (
      <Container>
        ...
        <Row>
          <Label>Amount</Label>
          <Value>
            <MetValue>{converterLog.decoded.met}</MetValue>
          </Value>
        </Row>
