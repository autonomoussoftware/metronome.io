...
import AuctionCopyClipboard from './CopyClipboard'
...
class AuctionBuyOptions extends Component {
  ...
  render() {
    const { auctionsAddress, hideBuyPanel, showBuyForm } = this.props
            ...
            <div className="panel__buy-metronome --showBuyMet">
              ...
              <section>
                <a
                  ...
                  {...(isWeb3Available
                    ? { onClick: showBuyForm }
                    : { href: useWallet.url, target: '_blank' })}
                >
                  <span className="btn-text">
                    Buy with {isWeb3Available ? web3Provider : useWallet.name}
                  </span>
              ...
              <section className="auction__option-section">
                <DownloadWalletForm />
                <div className="auction--or-oval">
                  <span className="auction--or-text">or</span>
                </div>
              </section>
              <section className="auction__contact-address">
                <h2>Buy With Your Own Wallet</h2>
                <p>
                  To make a purchase, send ETH to the address below. Make sure
                  the address you use is that one. We recommend copying it or
                  scanning the QR code.
                </p>
                <AuctionCopyClipboard address={auctionsAddress} />
              </section>
              <section
                className="auction__qr-code-scanner"
                style={{ textAlign: 'center' }}
              >
                <div className="auction__qr-code">
                  <img
                    alt=""
                    src={`https://chart.googleapis.com/chart?chs=160x160&cht=qr&chl=${auctionsAddress}&choe=UTF-8`}
                  />
                </div>
