...
function getAuctionStatus ({ auctions, metToken }) {
...
function createMetApiStream (url) {
  ...
  socket.on('AUCTION_STATUS_TASK', function (status) {
    ...
    status.tokensRemaining = status.tokenRemaining.toString()
    status.tokenSupply = status.tokenCirculation

    // Fix property types
    status.currentAuction = toInt(status.currentAuction)
    status.currentPrice = status.currentPrice.toString()
